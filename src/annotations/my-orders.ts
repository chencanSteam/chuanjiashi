...
② 按钮可见性：去支付 = 待支付且未过期；申请退款 = 已支付或服务中；评价 = 已完成且未评价过；查看详情打开详情弹窗。
③ 去支付走 paymentApi.pay（微信），成功后跳转 /order-success?orderId=xx；支付中按钮禁用防重复。
④ 传记师订单卡片：状态九态（待付定金 → 已付定金 → 已预约采访 → 已提交初稿 → 修改中 → 已提交终稿 → 已付尾款 → 已完成 → 售后中）着色展示；服务进度按 6 个固定节点（支付定金 → 交付定稿）的完成数计算百分比。`,
    },
    {
      id: 'my-orders.detail-modal',
      target: '订单详情弹窗',
      logic: `① 按订单字段分区展示：收货地址（仅实体订单）、物流信息（公司 / 运单号 / 发货时间）、交付物（PDF / 视频 / 二维码 / 链接 / 图片，可新窗口打开）、我的评价、售后记录。
② 订单号支持一键复制；底部操作按钮规则与列表一致（待支付可立即支付、已完成可评价、已支付/服务中可申请退款）。`,
    },
    {
      id: 'my-orders.review-modal',
      target: '评价弹窗',
      logic: `① 星级默认 5 星、可点选 1-5 星；评价内容必填，空内容拦截提示。
② 提交 orderApi.review（状态初始为 pending 待审核）后关闭弹窗并刷新列表；订单标记已评价，之后不再出现评价入口。`,
    },
    {
